...
and scintillation measurements are concrete evidence for radio waves having
...
expected values. Comparing obtained $\tau$ values to values predicted by current
electron density models for the Milky Way (e.g.
\citealt{2002astro.ph..7156C,2017ApJ...835...29Y}) provides further proof for
its extra galactic nature.

Similarly, pulse scintillation is evidence for passage through an inhomogenous
medium.  Scintillation estimates are obtained by first computing the
auto-covariance function of the burst's spectrum, and then the  decorrelation
...
1/(2\pi\Delta \nu_d)$.  Though, as seen in the XAO event (Section
\ref{sec:xao_event}), frequency modulation can appear similar to scintillation.
...
information.  These encodings schemes will appear different from polarized
...
% FRB 010724: Lorimer burst, the original and is thus the archtype

% FRB 090625: prototypical with potential scattering

% FRB 110523: No GBT public data? Data is available. I have fits file - Marisa.
...
% TODO: 861 dedispersion time series
\begin{figure}
...
\cM{The one-off nature of \glspl{frb} makes it essential to do sufficient due-dilligence \sout{that} when reporting on a
detection or triggering a follow-up observation \sout{that significant due-diligence
...
filter for dispersed pulses. As discussed in Section, \ref{sec:verify_crit} \sout{there
are} numerous additional tests \sout{that} can be performed to verify a detection as
astrophysical. The basic criteria can be fulfilled \cM{met?} using the dynamic spectrum
from the search pipeline. But, many of these criteria require capturing more
extensive data. It is not always possible to \cM{capture} sufficient data to \cM{test} every
\cM{criterium}, nor is it possible with every observational setup. \sout{But, being able to
test more criteria will lead to higher confidence in reporting an event as
astrophysical.} \cM{Obvious?}

\subsection{Minimum Reporting}

If most \glspl{frb} are indeed one-off events, then the detection data \cM{are \sout{is}} the
only data that will \sout{ever} be available.  At a minimum, a reasonable detection
should be reported with \sout{the observed} data made publicly available.  This allows
independent verification, and can be used as input data sets to test on
independent pipelines.  \sout{A} Minimum reporting should include:
...
\sout{More} Desirable reporting includes:
...
\begin{enumerate}
    ...
    interferometric baselines. \cM{Where will all the acronyms be defined? In a glossary at the end?}
...
\cM{A dynamic spectrum extended in time,}
\sout{An extended time duration of dynamic spectrum} can be used to \sout{look for} \cM{find} gain and
band pass variations. Low-\gls{snr} pulse searches can be performed to \sout{see} \cM{test} if
...
\cM{It allows for} coherent dedispersion \sout{can be done to} \cM{which} increases the detection sensitivity as in
\sout{the case of} FRB180301 \citep{atel11376} \sout{which} where \sout{revealed} an apparently narrow-band
detection \cM{was revealed} to have low-level flux across a larger band. The time and frequency
...
should be made \sout{in} to do so. \cM{I think it's contradictory to say all criteria don't need to be checked, but then to say an effort should be made to do so. I would just say not all can be checked, but an effort should be made to check as many as possible.}
...
During a search for anomalous signals \sout{it should be expected that there are} \cM{we expect} a
number of \sout{events} reported \sout{which are} false-positives events, such as Perytons
\citep{2011ApJ...727...18B} and the events presented \sout{reported} in Section
...
explanation they are hard to discount as \textit{not} being of astrophysical origin. It
