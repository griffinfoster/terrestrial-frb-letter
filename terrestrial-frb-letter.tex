...
All the examples of false-positive detections presented in this section do not
follow a cold plasma dispersion relation. But, as has been shown, pulses with a
range of frequency modulations will still be detected in an \gls{frb} search
pipeline.
% TODO: why is this section here? what is the point?

The \gls{snr}-maximized DM of a detected pulse is due to not just its frequency
structure, but also the receiver system.  Knowing the broadband structure of the
source, we can build a simple model to show that different dispersion relations
can produce a range of apparent \gls{snr}-maximized events depending on the
observing band compared to the pulse transmission band.

Pulses are modelled with a linear ($\nu^{-1}$), quadratic ($\nu^{-2}$), cubic
($\nu^{-3}$), and logistic function which covers a bandwidth from
$\nu_{\textrm{p,0}} = 1350$~MHz to $\nu_{\textrm{p,1}} = 1750$~MHz, example
pulses are shown in Figure \ref{fig:sim_pulse_spectra}. The time duration of the
pulse $\Delta t_{\textrm{pulse}}$ is fixed such that central frequency range of
the pulse has an \gls{snr}-maximized DM of approximately 1000~pc~cm$^{-3}$. The
pulse is given a width ($\Delta t_{\textrm{width}}$) by convolving the logistic
function with a 3~ms wide Gaussian. The amplitude of the pulse is set to unity
across the extent of the band.
...
bandpass is modelled as a Gaussian. A schematic of the pulse and receiver model
is shown in Figure \ref{fig:simulation_diagram}.
...
a power law spectral index \cMay{should we put frequency scaling index to avoid
confusion with the flux density spectral index?} $\alpha =4$ or $4.4$ (e.g. 
\citealt{Rickett1977}), and observations of pulsars often showing $\alpha < 4$.  
Spectral indices computed from broad band FRBs, should be compared to these expected values.
...
What is the \gls{snr}-maximized \gls{dm}? How does this compare to the modeled Galactic
DM along the line of sight? Is it well in excess of the Galactic
...
multi-component pulse profile \cMay{need reference here}. 
...
of both of these detections could be due to scintillation \cMay{or plasma lensing?}. 
These low scores do not discount FRB140514 and FRB180301 as astrophysical, they indicate
that FRB140514 and FRB180301 diverge from the prototypical \gls{frb} model.  We note
...
lacking in data or \cMay{detailed?} analysis. For example, very little information on
FRB171209 has been reported, thus almost none of the criteria can be tested.  Providing
...
\vspace{1pt}
\noindent  ALFABURST  activities  are  supported  by  a
National  Science  Foundation  (NSF)  award  AST-1616042. MPS and DRL acknowledge support from NSF RII Track I award number OIA--1458952.
