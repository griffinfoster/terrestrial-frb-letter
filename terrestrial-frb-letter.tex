...
not a false positive. Currently, there is currently no formal validation
framework for FRBs, rather a set of community practices. In this article, we
discuss potential sources of false positives, and suggest a framework in which
FRB-like events can be evaluated as real or otherwise.  We present examples of
false-positive events in data from the Arecibo, LOFAR, and Nanshan telescopes,
which while FRB-like, are found to be due to instrumental variations, noise, and
radio-frequency interference.  Differentiating these false-positive detections
from astrophysical events requires knowledge and tests beyond thresholded
...
of the detections.  Adopting a \gls{sefd} of 3~Jy for the \gls{alfa} receiver
\GSF{ref},  results in \mbox{$S = 66$}~mJy from beam 0, which would be lower
than what is observed for any previously detected \gls{frb}
\citep{2016PASA...33...45P}.  However, this flux estimate is a lower limit,
assuming the source is at the center of the beam. The pulse width is large for
an \gls{frb} but within the range of those previously reported.
...
\dcp{I think a short summary of Perytons at Parkes would be valuable here. Two
refs are already in text, Petroff: \citep{2015MNRAS.451.3933P} and
Burke-Spolaor: \citep{2011ApJ...727...18B}. Example text: A class of
non-astrophysical yet FRB-like signals were discovered in high time resolution
data around 1.4\,GHz with the Parkes telescope. These signals, dubbed
`Perytons',are swept in frequency like FRBs, but do not obey the $\nu^{-2}$ cold
plasma relation. Ultimately, these were found to be a form of RFI generated by
microwave ovens. mention which diagnostics are useful for discerning them,
e.g. time of day dependence, coincidence with 2.3--2.5\,GHz transmission.}
