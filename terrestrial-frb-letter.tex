...
analysis and data which should be provded when reporting a detection.
\end{abstract}
...
emission model is not known. This consenus has been built from the reporting of
...
To detect these rare events automated, GPU-accerlated software pipleines are
...
We expect a number of false-positives to pass automted post-processing detection
...
\end{figure}
%
The exact physical process which generated is additonal system noise is not
...
As the automated search pipeline is setup to do an extensive search in DM
trails, pulse width, and starting time it is reasonable to expect a large number
of low-SNR false-detections.  FRB surveys tend to have a significant minimum SNR
cut-off (6-10) due to the large number of false-postive detections at low SNRs.
In the ALFABURST and ARTEMIS surveys the minimum SNR cut-off is set to 10.

Low-SNR detections are regularly detected even with high mimimum SNR cut-off.
...
has a slightly higher SNR but is visable in the spectrogram. The solution is
...
seperated by approximately 10 ms, with the second component appearing only about
...
\begin{figure}
    ...
    components distinct components seperated by approximately 10 ms. Data is
...
Further, many anthropogentic radio transmitters are circularly polarized. But,
...
During a search for anomolous signals it should be expected that there are a
...
information, and lack of an external, anthropogentic explaination they are hard
to discount as not being of extra-terrestrial origin. It is neccessary to report
...
mystery and rarity of FRBs makes it difficult to differntiate astrophysical FRBs
...
% -> many FRBs have this on FRBCAT
% minimum detection requirements:
%   SNR maximized DM, time and freq resolution
%   filterbank covering the extent of the pulse (online)
%   scipt/guide to recreate detection
%   astronomical pointing

% desirable requirements:
%   filterbank +/- multiple minutes of event
%   expected and observed bandpass model, SEFD/gain variations
%   telescope observing status +/- multiple minutes of event
%   telescope local pointing
%   DM-space plot (with negative DMs)
%   dispersion fit
%   raw voltage data, at least undecimated power spectra

% exemplary requirements:
%   multi-site observations
%   increased bandwidth (BL C-band obs)
%   interferometric visibilities

\subsection{Negative DM Sampling}
...
for an extra-terrestiral origin when reporting a detection. 
...
extra-terrestrial origin, just as the explaination for perytons
