...
always follow a cold plasma $\nu^{-2}$ relation.  This rules out a pulsar, or an
astronomical source in general.
...
%The computed DM value which maximizes the \gls{snr} of a burst detection, is a
%function of both the pulse dispersion relation and the response of the receiver
%system. A pulse with a dispersion relation different from the dispersion by a
%cold plasma ($\nu^{-2}$), can therefore still be detected by the search pipeline
%with a significant \gls{snr}.
...
%is the result of the pulse dispersion relation and the response of the receiver
%system. A pulse with a dispersion relation similar to dispersion by a cold
%plasma will still be detected in an \gls{frb} search pipeline with a significant
%\gls{snr}.

We model this effect by simulating a number of dispersion and receiver models at
L-band.  Pulses are modeled with linear ($\nu^{-1}$), quadratic ($\nu^{-2}$),
cubic ($\nu^{-3}$), and logistic ($\frac{1}{1+e^{-\nu}}$) functions which covers
a bandwidth from $\nu_{\textrm{p,0}} = 1300$~MHz to $\nu_{\textrm{p,1}} =
1800$~MHz.  The quadratic dispersion model is used as reference, while the other
...
G.F. and D.C.P. acknowledge support from the Breakthrough Listen Initiative.
