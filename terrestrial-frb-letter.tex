...
radio-frequency interference \sout{(RFI)}.  Differentiating these false-positive
...
anthropogenic radio \cM{signals}, variations in the observing system, and natural
signals from within the solar system.

Most false-positive events are \cM{\sout{filtered out with} removed using?} filters and classifier models.
The remaining events are commonly examined by eye as expert human knowledge
appears to be the best way \cM{(as of yet)} to classify a detection \sout{but} \cM{However,} time limitations\cM{/constraints?} \sout{in time} do
not allow for all events to classified \cM{in} this way. On inspection, true
astrophysical events and terrestrial events can be difficult to differentiate \cM{between,?}
even by the expert.  These terrestrial events, indeed, should appear like
astrophysical \glspl{frb} as they have passed the detection pipeline tests. 

\cM{Studies of the telescope state during detection, can often assist in determining the origin of a mysterious event. Nonetheless, ambiguities can remain, leading to the precarious, often minimalist, way in which frbs have been reported on.  my version. up to you.}   


\sout{By \cM{hmm By at the start}
...
of reporting an \gls{frb} as astrophysical.}


In this section we present examples
of such events \cM{--} which on initial inspection appear to be astrophysical, but after
...
the \gls{frb} search pipeline \cM{flagged? saved?} detected \sout{an} events using a \sout{above the} minimum \gls{snr}
detection threshold of 10. \cM{(singular event made it sound like one was detected)} The vast majority of these events were due to
...
density of \mbox{$S = 66$}~mJy from Beam 0, which would be \sout{a} lower \sout{flux} than for any
previously detected \gls{frb} \citep{2016PASA...33...45P}. This flux estimate \cM{however?} is
a lower limit, as we are assuming the source is at the centre of the beam. \cM{Added however to show the we understand the statements are else contradictory: lowest ever, but just a lower limit} The
...
($S/N=16$) for a DM of 829~pc~cm$^{-3}$. \sout{Upon further inspection and} \cM{By} testing
different DM \cM{values} we found that this event appeared to decrease in width for lower
DM trials. \sout{There was} \gls{rfi} clipping \cM{was applied} \sout{in} \cM{during} this event which is known to
...
similar to that of a single pulse from a high DM pulsar\cM{,} such as PSR B1859+03
...
event. No known pulsar or \gls{rrat} with a similar DM \sout{is nearby at} \cM{lies within?} this pointing.

\cM{The telescope l}ogs provide the first indication that this event is due to a local source.  \cM{strange tenses, went from telescope was pointed to logs provide - past to present} As
...
different position \sout{to that of} \cM{than} where ALFA should be if it was \cM{were? subjunctive mood?} at the secondary
focus.

The \cM{observing?} observation schedule for the morning of December~4, was project
...
local time.  Followed \cM{would make it one sentence Hemingway} by Project
R3037\footnote{http://www.naic.edu/vscience/schedule/tpfiles/TaylortagR3037tp.pdf},
an S-Band RADAR observation.  The event \sout{appear to have} occurred during a time of
no active observation, \sout{otherwise, \gls{alfa} would not have been active and in
the wrong turret position.}\cM{causing alfa to be active but in the wrong turret position?}
...
bandpasses appear similar and have overlapping narrow-band RFI features \sout{that
...
increase in system noise. \cM{going into? rather transitioning to/ becoming non-linear, having a non-linear system noise dependence?}
...
\sout{The theory that this event is due to an} \cM{Evidence for analogue gain variations} is further justified
by looking at \cM{considering?} data in \cM{over?} a larger time window.  Approximately 80 seconds earlier \sout{a
window was recorded which had} \cM{data show large} structures across the band (Figure
...
studying the telescope state and examining data \cM{across} \sout{in} a larger time window around
...
false-positive detection based on a \sout{confluence of multiple factors} \cM{unique telescope state?}, \sout{there are}
other false-positive events \sout{which} occur regularly.  As automated search
...
limit the number of these false-positives, \sout{but some of course past this minimum
cut-off threshold.} \cM{-obvious?}
...
determined that this event was due to an adjustment in the Arecibo receiver turret
position. \cM{sounds somewhat similar than the previous event of section 2.1} But, this is not an uncommon event and many of these events have no
\sout{obvious} \cM{immediate} explanation.
...
terrestrial. If sufficient data are \sout{is} not recorded during a detection, \sout{there will be
insufficient} evidence to be confident about the event's origin \cM{will be lacking}. The choice of
...
from FRB121102 with APERTIF \citep{atel10693} had an \cM{a?} \gls{snr}$\approx 4$. In a
blind survey across different sky positions and DM trials this would not be a
significant detection. But, the sky position and \gls{snr}-maximized DM of \cM{the repeating}
...
145~MHz.  In this survey, known pulsars regularly transit the beam that are \cM{is?}
fixed in local coordinates, and single pulses are routinely observed.  An
\gls{rfi} \cM{clipper, tool, mask?} has been developed for this survey that successfully removes the
...
\begin{figure}
    ...
    at the LOFAR-UK station. The \gls{snr} is maximized when \cM{data are/the signal is} dedispersed by a DM
...
\begin{figure}
    ...
    apparent events during \cM{\sout{at}} the time.
    }
...
determine the exact origin of the RADAR pulse, \cM{since? you have As right after this} RADAR is used for commercial
...
% RADAR for tracking a geostationary satelite launch?
% extreme range distance 0-~30000km based on time delay
% repeating nature to keep constant track
% once-off nature means it happened only once
% Jiuquan Satellite Launch Center not far from XAO. unreported satellite launch?
%
% Likely answer:
...
"Similarity to the archetype" and "Terrestrial \cM{in?} origin". For each criterion, we
propose 6 possible answers, which we colour-code in our own categorization
below. Answer 1 to 4 encapsulate the responses: ``Identical", ``Similar", ``Not
similar", ``Completely different", while 5 and 6 are used for the responses ``Test
not performed" and ``Data not available" respectively.  \cM{Fixed double quotes. Can consider italics instead?}
...
hundreds of microseconds (FRB150807, FRB170827) down \cM{? down, isn't it up?} to tens of milliseconds
...
present, as in the case of RADAR \cM{contamination?}(Section \ref{sec:LOFAR_RADAR}). The dynamic
spectrum should be checked \cM{for sub-structure} at both the highest time and frequency resolution \sout{to
look for structure}. Complex voltages allow for coherent dedispersion and
...
Are there multiple components to the pulse? \sout{How many components are there?} \cM{And how many?} Does
this \cM{number} \sout{differ by} \cM{change when} dedispersing with different DMs? Most \glspl{frb} appear as
single component pulses. But, a few, such as FRB130729 and some bursts from
FRB121102, show multiple components, which can lead to differences in the
`optimal' DM \citep{2018Natur.553..182M}. Pulses from FRB121102 often contain
multiple components. As such it deviates from the prototypical. As FRB121102 is
known to be astrophysical, this criterion can not invalidate a detection.
...
the (mostly unknown) position of a detected pulse within the primary beam, there
will be a frequency-dependent sensitivity response, which will induce spectral
colorization in a broadband source. If the pulse appears band-limited with a
steep spectral index, this could indicate the pulse flux drops below the system
noise in parts of the band. If a shallow spectral index is fit to a band-limited
pulse, this could indicate the pulse is intrinsically band-limited.

\paragraph{Scattering:}

Does the pulse appear to be scatter broadened by the ISM, as in the case of e.g
FRB130626 \cite{2016MNRAS.460L..30C}? If so, can the scattering broadening be
...
fitted to the data? Most ISM models predict high spectral indices with $\alpha
=4$ or $4.4$, (e.g. \citealt{Rickett1977}). Frequency-dependent scattering is
strong evidence for the propagation of radio waves through an inhomogenous ISM,
and hence for sources to be astrophysical. 
% TODO: reference FRB
...
% TODO: Marisa, can you make this point stronger? how does the scintillation
% bandwidth scale?
% TODO: reference FRB, FRB 180301

\paragraph{Polarization Characteristics:}

Were full Stokes parameters measured for the pulse? \cM{or coherency values XX YY?} Does the pulse show
...
characteristics, such as Phase-shift keying \cM{weh? what is keying?}, as a way of encoding information.
These encodings schemes will appear different from polarized astrophysical
sources.
% TODO: was the GBT one circularlly polarized? FRB140514 was

\paragraph{DM Excess:}

What is the \gls{snr}-maximized \gls{dm}? How does this compare to \cM{the} Galactic DM
...
showing a $\nu^{-2}$ relation is evidence for a dispersed, nature source.
...
event then the source is terrestrial.
...
system in order to test this criterion with due diligence. A test of the overall
telescope state would be to observe a known pulsar regularly during
observations.
...
components \sout{can be seen to be} \cM{is} more narrow than the reported $\sim 16$~ms.  The
two components are separated by approximately 10~ms, with the second component
appearing only about half as wide in bandwidth compared to the first. \cM{probably good idea to use approximately or $\sim$ consistently}

The limited bandwidth and repeating structure \cM{do you mean multiple peaks?} of the event can be taken as
evidence \sout{that it is} \cM{for} a signal from an artificial source. The pulse shows a sharp
...
explanation is that this is a signal from a band-limited RADAR. \cM{That's not really alternative, RADAR is an artificial source} Though, it is
not possible to show \sout{with} \cM{which} model is correct from the available data.
...
\begin{figure}
    ...
    \cM{How do you feel about ticks on the time axes?}}
    \label{fig:FRB130729}
\end{figure}

\subsubsection{FRB 140514}

\cM{The} detection of FRB140514 was reported \sout{in Petroff}.  \sout{The event was reported} to have significant circular polarization \citep{2015MNRAS.447..246P}. \sout{though t} This is not
\sout{necessarily an indication} \cM{unambiguous evidence for?} of an astrophysical source, since many artificial
...
fractional bandwidth is insufficient to fit a dispersion relation. Taking \cM{Considering? I don't like taking}
...
$\alpha=-2$ relation \cM{have we defined alpha in this way, I don't think so. I have defined alpha for scattering spectral index. Here I would just right spectral index of 2, I think}. The band-limited nature of the pulse and the apparent
frequency modulation\cM{,} similar to the XAO repeating sources\cM{,} indicate the event
...
The only verified \gls{frb} is the repeating source\cM{,} FRB121102. Events have been
...
known to be astrophysical. \cM{going to come back to the last sentence again, to make sure the point is clear. I think it could else be easily criticized}
...
detections discussed in Section \ref{sec:false-pos}. We \sout{also} \cM{further} include
...
%The right columns of Figure \ref{fig:heat_map} show the type and
%completeness of data publically available.
% TODO: comment on the data that has been made available. make statement about
% color scheme used, or maybe drop the right column
